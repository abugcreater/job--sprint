...
export function ApplicationForm({
  ...
  onRecord: () => void;
	    ...
	    <section className="command-panel" aria-labelledby="application-form-title">
	      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	        <div className="flex items-center gap-2 text-brand-700">
	          ...
	          <h2 id="application-form-title" className="text-base font-black text-ink-900">
	            {isEditing ? "编辑机会记录" : "新增机会记录"}
	          </h2>
	        </div>
	        {isEditing ? (
      ...
      <TextareaField label="沟通反馈" value={draft.hrFeedback} onChange={(value) => onChange({ hrFeedback: value })} placeholder="记录招聘方原话、约面时间、补资料要求或拒绝原因。" minHeight="min-h-[104px]" />
