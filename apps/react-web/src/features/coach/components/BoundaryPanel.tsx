import { AlertTriangle, CheckCircle2, Edit3, Lightbulb, RotateCcw, Trash2, XCircle } from "lucide-react";
...
function BoundaryDiscardChangesDialog({
  actionLabel,
  onContinue,
  onDiscard
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-ink-900/40 p-4 backdrop-blur-[1px] sm:items-center sm:justify-center" role="presentation">
      <section className="w-full max-w-md rounded-card border border-line bg-white p-5 shadow-soft" role="alertdialog" aria-modal="true" aria-labelledby="discard-boundary-changes-title" aria-describedby="discard-boundary-changes-detail">
        <div className="flex items-start gap-3">
          <span className="grid size-10 shrink-0 place-items-center rounded-control bg-warn-100 text-warn-600">
            <AlertTriangle size={20} aria-hidden="true" />
          </span>
          <div className="min-w-0">
            <h2 id="discard-boundary-changes-title" className="text-lg font-black text-ink-950">放弃未保存的知识边界修改？</h2>
            <p id="discard-boundary-changes-detail" className="mt-2 text-sm font-semibold leading-6 text-ink-600">你对当前知识边界的本次修改尚未保存。继续编辑会保留输入；放弃后将{actionLabel}。</p>
          </div>
        </div>
