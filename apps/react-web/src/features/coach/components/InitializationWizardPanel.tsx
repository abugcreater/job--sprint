...
export function InitializationWizardPanel() {
  ...
  const handleApplyTemplate = () => {
    ...
    setMessage(`已套用「${template.label}」首登模板。`);
  };
  ...
  const handleExtract = () => {
    if (!canSaveProfile(profileDraft)) {
      setMessage("快速初始化需要目标岗位、经验摘要和每日分钟。");
      return;
    }
    ...
    setMessage(nextSuggestions.length ? "已生成初始化边界候选。" : "未识别到新的边界候选，可以换一段素材。");
  };
  ...
  return (
    <article id="coach-quick-init" className="command-panel scroll-mt-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <PanelTitle icon={<Sparkles size={18} aria-hidden="true" />} title="快速初始化" />
          <p className="mt-2 text-sm font-semibold leading-6 text-ink-500">用首登模板和批量素材包生成首批边界，再落成今天的第一条行动。</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="block">
          <span className="text-sm font-black text-ink-700">首登模板</span>
          <select className="field-control mt-2" value={templateId} onChange={(event) => setTemplateId(event.target.value as ProfileRoleFamily)} aria-label="首登模板">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Field label="快速目标岗位" value={profileDraft.targetRole} onChange={(value) => setProfileDraft((current) => ({ ...current, targetRole: value }))} placeholder="Java 后端 / 前端 / 测试开发" />
        <label className="block">
          <span className="text-sm font-black text-ink-700">快速角色族</span>
          <select className="field-control mt-2" value={profileDraft.roleFamily} onChange={(event) => setProfileDraft((current) => ({ ...current, roleFamily: event.target.value as ProfileRoleFamily }))} aria-label="快速角色族">
            {profileRoleFamilies.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}
          </select>
        </label>
        <Field label="快速每日分钟" value={profileDraft.dailyMinutes} onChange={(value) => setProfileDraft((current) => ({ ...current, dailyMinutes: value }))} />
        <Field label="快速不可夸大边界" value={profileDraft.nonClaims} onChange={(value) => setProfileDraft((current) => ({ ...current, nonClaims: value }))} placeholder="不能包装的经历" />
      </div>
      <Textarea label="快速经验摘要" value={profileDraft.experienceSummary} onChange={(value) => setProfileDraft((current) => ({ ...current, experienceSummary: value }))} placeholder="当前经验、主线能力、最近项目。" />
      ...
      <Textarea label="初始化素材" value={sourceText} onChange={setSourceText} placeholder="粘贴 JD、简历片段、面试反馈或学习笔记；多段素材可用 --- 分隔。" />
