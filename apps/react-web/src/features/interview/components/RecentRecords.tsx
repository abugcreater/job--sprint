  ...
  return (
    <article className="command-panel">
      ...
      {records.length ? (
          ...
          {records.map((record) => (
            <div key={record.id} className="rounded-card bg-surface-0 p-3">
              ...
              {record.scoreSummary ? (
                ...
                <p className="mt-1 rounded-control bg-warn-100 px-2.5 py-1 text-xs font-black text-warn-600">未自检</p>
              )}
        ...
        <p className="mt-3 text-sm font-semibold leading-6 text-ink-500">暂无口述记录。写一版回答后保存为 Evidence Gate 证据。</p>
      )}
    </article>
  );
}
