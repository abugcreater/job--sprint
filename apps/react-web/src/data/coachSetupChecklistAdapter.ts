...
export function buildCoachSetupChecklist({
  ...
  const steps: CoachSetupStep[] = [
    setupStep("profile", "求职画像", activeProfile ? `${activeProfile.targetRole || activeProfile.roleFamily} · ${activeProfile.dailyMinutes} 分钟/天` : "未保存目标岗位和时间投入", activeProfile ? "检查画像" : "导入简历", "coach-profile", profileReady ? 1 : 0, 1),
    ...
    setupStep("ai_review", "AI 建议确认", `${Math.min(reviewedArtifacts, 1)}/1 条已接受或拒绝`, artifacts.length ? "处理建议" : "生成建议", "coach-artifacts", Math.min(reviewedArtifacts, 1), 1)
  ];
  ...
  return {
    ...
    summary: nextStep ? `还差 ${totalCount - completedCount} 项，下一步处理「${nextStep.label}」。` : "建档完成，后续 AI 建议会基于你的画像、边界和日程持续迭代。",
