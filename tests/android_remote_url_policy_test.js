...
assert.match(policy, /static boolean isAllowedWebViewUrl\(String url\)/);
assert.match(policy, /url\.startsWith\("file:\/\/\/android_asset\/"\)/);
assert.match(policy, /isAllowedRemoteSchemeAndHost\(parsed\)/);
...
assert.match(remoteWebViewClient, /RemoteUrlPolicy\.isAllowedWebViewUrl/);
assert.doesNotMatch(activity, /RemoteUrlPolicy\.isAllowedWebViewUrl/);
assert.match(basicAuthController, /RemoteUrlPolicy\.isAllowedRemoteHost/);
