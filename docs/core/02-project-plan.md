...
- 目标：让正式域名在不暴露求职数据的前提下，向备案审核人员和普通访客清楚展示网站名称、个人非经营性定位、服务边界、隐私政策、用户协议、ICP备案信息和公安联网备案状态。
- 工作分支：`codex/feature/public-security-filing-home`。
- 来源分支：`develop`。
- 目标分支：`develop`。
- 实现边界：根路径改为公开的“日程回声谷（Job Sprint）”介绍页；`/react/`、`/schedule.html`、`/data/` 和业务 API 继续要求登录；不开放注册、用户数据或管理能力。
- 提交计划：页面与路由使用 `feat(compliance): add public filing-ready website surface`；测试和事实源随同一需求收口，不混入其他产品功能。
- 验证计划：Node 与 Rust 路由测试、公开包敏感扫描、未登录/已登录权限回归、桌面和移动浏览器检查、正式域名 HTTPS/登录/API/HTTP 跳转冒烟验证。
- 部署计划：使用现有 public-safe 与 Linux Rust server-delivery 链路同步服务器；同步前保留可回滚发布物，部署后以正式域名和服务器侧证据为准。
- 外部边界：公安联网备案实名登录、证件上传、短信或人脸核验和最终提交必须由备案主体本人完成；审核通过后再使用平台下载的正式备案图标与 HTML 代码替换页面占位状态。
