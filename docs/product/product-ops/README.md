...
| `gitflow-development-governance.md` | 需求分支、PR 目标、发布与 hotfix 的 GitFlow 规范。 |
...
1. 新需求先看 `gitflow-development-governance.md` 和 `requirement-development-template.md`，从正确基线创建独立分支，再建 `docs/product/<feature-slug>/`。
...
5. PR 前运行 `npm run validate:gitflow` 和影响范围门禁；稳定分支只通过合规 PR 更新。
