...
- `v0.2.4` 于 2026-07-26 发布；开始时仅有 #32、#33 两项新增需求，未达到 7 天或累计 3 项需求阈值。本轮 #34 合入后成为第三项，按 GitFlow 创建 `release/v0.2.5`；Release PR #35 已合入 `main` 并在合并点创建附注标签 `v0.2.5`，随后通过本回同步 PR 返回 `develop`。这是源码发布、标签和回同步流程，不执行服务器交付。
