...
- #45 已经 GitHub `validate` 通过后 squash 合入 `develop`（`9d55dc4`），并删除需求短分支；#46 已通过发布门禁、merge 至 `main`（`8b2654d`），`v0.2.7` 已创建并推送。当前回同步 PR 仅带回发布提交与收口记录；服务器交付仍需用户单独授权。
...
- 本项已作为 `v0.2.5` 之后第三项需求合入 `develop`，按 GitFlow 创建 `release/v0.2.6 -> main`，完成标签和 `main -> develop` 回同步；不运行未获授权的服务器交付。
...
- 已执行 `git fetch --prune origin`；开始时工作树干净，`develop` 与远端一致，没有目标为 `develop` 的开放 PR、Draft、冲突或短分支积压。
- `origin/main` 是 `origin/develop` 的祖先，文件树差异来自已合入功能，不是回同步阻塞。
- `v0.2.3` 于 2026-07-23 发布；开始时只有 #27 和 #28 两项新增需求，未达到 7 天或累计 3 项阈值。本次需求合入 `develop` 后会成为第三项，随后按 `release/* -> main` 流程准备 `v0.2.4`，不运行服务器交付。

### 今日选择：机会编辑未保存修改保护

| 维度 | 评分 | 依据 |
|---|---:|---|
| 用户价值 | 4/5 | 机会记录承接公司、岗位和沟通事实，取消时直接丢失输入会打断用户的真实求职记录。 |
| 确定性 | 5/5 | P2 机会工作台明确记录该缺口，代码中取消和移动端关闭都直接重置草稿。 |
| 风险降低 | 4/5 | 确认前不写入 Evidence Gate，避免误退出导致用户重新输入或误以为已保存。 |
| 交互改善 | 5/5 | 用户可明确选择继续编辑或放弃，未修改仍保持一步退出。 |
| 可验证性 | 5/5 | 可用适配器、页面、全量回归和桌面/窄屏浏览器走查验证。 |
| 实现范围 | 4/5 | 只覆盖机会编辑器的两个退出控件，不建立未经设计的全局表单框架。 |

### 改动

- 新增 `application-unsaved-changes` feature capsule，先比较局部确认、浏览器离页拦截和全局协议三种方案，裁决为机会编辑器局部确认。
- `applicationsAdapter` 新增草稿克隆和精确 dirty 判断，标签数组不会与基线共享引用。
- 机会页把真正关闭编辑器与请求关闭分离：有修改时显示“继续编辑 / 放弃修改”，继续编辑保留草稿，放弃才重置草稿、返回详情或列表；保存、删除确认和 Evidence Gate 写入逻辑不变。
- 更新 P2 机会工作台边界、产品账本和已知问题，避免将局部能力表述成全局表单保护。

### 验证

- `npm --prefix apps/react-web run typecheck` 通过。
- 定向机会测试通过：2 个文件、12 个测试。
- `npm test` 通过；React 为 37 个文件、120 个测试，产品迭代、敏感扫描、工作树边界和 GitFlow 测试均通过。
- `npm --prefix apps/react-web run build` 通过；仅保留既有的大 bundle 警告。
- 本地浏览器以独立 `localhost` 数据域走查默认桌面与 `390 × 844`：新增机会修改公司后取消出现确认，继续编辑保留输入，移动端关闭后放弃修改会关闭编辑器；控制台无 error。走查只写入本地测试画像，没有调用服务端。

### 限制与明日建议

- 本轮不拦截刷新、关闭标签页、应用强杀、Android 系统返回或全部外部路由；不实现全局表单协议、自动保存、服务端草稿、服务器交付或 Android 更新。
- 下一个产品候选是基于同一交互协议评估知识边界、日程和复盘表单，但必须先做跨模块状态与 Android 返回设计，不能直接复制本轮实现。

## 2026-07-25 第五十二次主动迭代

### GitFlow 基线

- 已执行 `git fetch --prune origin`，开始时工作区干净，`develop` 与远端一致。
- `origin/main` 是 `origin/develop` 的祖先，二者文件树不同是已合入的增量功能所致，不是回同步阻塞。
- 没有目标为 `develop` 的开放 PR、Draft、冲突或短分支积压。
- `v0.2.3` 于 2026-07-23 发布；截至本轮只有 1 项已合入增量功能。即使本轮合入后也只有 2 项，未满足 7 天或累计 3 项需求的 release 条件，因此不创建 release 分支。

### 今日选择：AI 限流与超时诊断

| 维度 | 评分 | 依据 |
|---|---:|---|
| 用户价值 | 4/5 | 用户可区分“等待后再试”与“修正输入”，避免盲目连续生成。 |
| 确定性 | 5/5 | `429`、`408`、`504` 原先均被错误归为响应合同异常，修复点明确。 |
| 风险 | 4/5 | 仅改变客户端诊断语义和提示，不改正式写入、provider 配置或服务器。 |
| 交互收益 | 4/5 | Coach 即时反馈直接展示下一步，详情仍可在运行记录和统计页追溯。 |
| 可验证性 | 5/5 | 可用单测、构建、代理诊断和本地回环 `429` 浏览器走查验证。 |
| 实现范围 | 5/5 | 影响集中在 React 运行时映射、诊断文案、测试和产品台账。 |

### 改动

- HTTP `429` 映射为 `rate_limited`，HTTP `408` / `504` 映射为 `api_timeout`，其他 `5xx` 继续映射为 `api_unavailable`。
- Coach 生成失败后的即时反馈改为优先展示恢复动作；AI 运行记录和统计页继续展示原因、诊断码与完整下一步。
- 补充运行时客户端、运行记录面板、统计页测试；补齐恢复提示的实现边界与产品决策记录。

### 验证

- `npm --prefix apps/react-web run typecheck` 通过。
- 定向测试通过：4 个文件、13 个测试。
- `npm --prefix apps/react-web test` 通过：37 个文件、117 个测试。
- `npm --prefix apps/react-web run build` 通过；仅保留既有的大 bundle 警告。
- `npm run test:coach-runtime-diagnostic` 与 `npm run test:rust-coach-runtime-proxy-auth` 均通过。
- 使用仅绑定 `127.0.0.1` 的临时回环桩返回 `429` 完成 Coach 页面走查：无自动重试或正式写入，桌面和 `390px` 视口均完整显示“AI 服务正在限流：请等待片刻后再生成；不要连续点击或反复刷新页面。”，控制台无 error。

### 限制与明日建议

- 本轮不证明真实 provider 的限流、超时和其他 `5xx` 行为；没有实现 `Retry-After`、自动重试、熔断、服务器交付或 Android 更新。
- 明日建议优先收集真实 provider 的可脱敏运行时证据，再决定是否建立独立的恢复策略需求；没有证据前不承诺自动重试。
