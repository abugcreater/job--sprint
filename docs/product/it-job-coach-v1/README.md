...
- 远端交付：`npm run validate:delivery -- --allow-dirty` 仍为 FAIL，剩余原因是 Android 远端 URL 仍是 HTTP、Android HTTPS 真机 evidence 缺失、最终统一交付报告不是 PASS。公网 HTTPS 域名 DNS 和服务器本机 Nginx/cert 正常，但外部 443 TLS 握手仍 EOF。
