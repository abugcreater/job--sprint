...
| `product-ops/` | 长期维护入口 | 记录跨版本文档规则、产品账本、已知问题和可复用迭代工作流。 |
...
3. 迭代结束后必须同步 `product-ops/product-ledger.md` 和 `product-ops/known-issues.md`。
4. 影响项目定位、架构边界或验收口径的结论，必须同步回 `docs/core/`。
5. `npm run validate:delivery` 未通过时，只能说本地、Android 本地或 HTTP 演示已通过，不能说完整 HTTPS 生产交付通过。
